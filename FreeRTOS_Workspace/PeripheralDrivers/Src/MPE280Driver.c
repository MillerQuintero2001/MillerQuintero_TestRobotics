...
/** Función para leer los datos del MPE280 */
void mpe280_readData(uint8_t address, uint8_t * ptrBufferData, uint8_t dataSize){
	...
	/* Desactivamos el escalvo */
	spi_unSelectSlave(&handlerSpiPort);
}

/* Funcíon para escribir sobre el los registros del sensor MPE280 */
...
void mpe280_init(void){
	...
	handlerSpiPort.SPI_Config.SPI_frameFormat			= SPI_FRAME_FORMAT_MSB;
